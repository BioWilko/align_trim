...
def check_scheme_hashes(filepath, manifest_hash):
    ...
    if hash_sha256 != manifest_hash:
        sys.stderr.write(f"sha256 hash for {str(filepath)} does not match manifest")
        raise SystemExit(1)
...
def get_scheme(scheme_name, scheme_directory, scheme_version="1"):
    ...
    sys.stderr.write("could not find primer scheme and reference sequence, downloading")
    ...
    sys.stderr.write(f"\tRequested scheme:\t{scheme_name} could not be found, exiting")
